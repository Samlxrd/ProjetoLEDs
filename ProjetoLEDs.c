...
uint8_t red_value = 20; // Intensidade do vermelho
uint8_t green_value = 0; // Intensidade do verde
uint8_t blue_value = 5; // Intensidade do azul
...
int main()
{
    ...
    while(true) {
        // LED Vermelho piscando alternando estado a cada 2s. (Apenas teste)
        gpio_put(LED_RED, 1);
        sleep_ms(2000);
        gpio_put(LED_RED, 0);
        sleep_ms(2000);
...
static void gpio_irq_handler(uint gpio, uint32_t events){
    ...
    if (current_time - last_time > 100000) // Delay de 100ms para debounce
    {
